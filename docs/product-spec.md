...
| **SES Webhook Ingestor** | Recebe notificações da SES via Amazon EventBridge (delivery/bounce/complaint), normaliza o payload, mantém a correlação `sesMessageId → emailId` | [Implementado]                                                          |
...
| `POST`                         | `/webhooks/ses`                | SES Webhook Ingestor | Segredo compartilhado (header, via EventBridge API Destination) | Notificação da SES via EventBridge (delivery/bounce/complaint)                                      | `200` (ack)                                                                                                                                           | [Implementado]                                                       |
