...
import { BASE_BACKOFF_MS, MAX_RETRY_ATTEMPTS } from '../application/retry-backoff.ts'

/*
 * Derived from the same constants computeNextRetryAt uses, so raising BASE_BACKOFF_MS or
 * MAX_RETRY_ATTEMPTS moves this test's timeout along with the real backoff instead of leaving a
 * hand-computed number to drift out of sync and fail for a reason unrelated to the behavior under
 * test. Sum of 2^1..2^MAX_RETRY_ATTEMPTS backoffs = BASE_BACKOFF_MS * (2^(MAX+1) - 2); at today's
 * values (5000ms, 3 attempts) that's 10s + 20s + 40s = 70s before the DLQ publish fires.
 */
const EXHAUSTION_FLOOR_MS = BASE_BACKOFF_MS * (2 ** (MAX_RETRY_ATTEMPTS + 1) - 2)
const DLQ_POLL_TIMEOUT_MS = EXHAUSTION_FLOOR_MS + 15_000
const DLQ_TEST_TIMEOUT_MS = DLQ_POLL_TIMEOUT_MS + 5000
...
describe('Dispatch Worker end to end', () => {
  ...
  beforeAll(async () => {
    ...
    /*
     * The same SESClient production actually builds (ses-client.provider.ts), not a second one
     * constructed by hand here — a hand-built client duplicates the credentials/endpoint wiring
  ...
  it(
    'exhausts retries and routes to the DLQ when the sender is rejected at the SES layer on every attempt',
    async () => {
      const emailId = randomUUID()
      ...
      /*
       * EXHAUSTION_FLOOR_MS is the backoff chain's own floor (10s + 20s + 40s at today's constants)
       * before the DLQ publish fires. DLQ_POLL_TIMEOUT_MS carries margin over that floor for SES/Kafka
       * round-trip latency and first-subscription consumer group setup.
       */
      await vi.waitUntil(() => dlqMessages.some((message) => (message as { emailId: string }).emailId === emailId), {
        timeout: DLQ_POLL_TIMEOUT_MS,
        interval: 500
      })
